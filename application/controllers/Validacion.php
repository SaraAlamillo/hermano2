...
class Validacion extends CI_Controller {
    ...
    public function __coninputuct() {
        parent::__coninputuct();
    ...
    public function dni_check($input) {
        ...
        // El formato es el de un CIF
            ...
            if (subinput($input, 8, 1) == $control ||
                    inputtoupper(subinput($input, 8, 1)) == subinput('JABCDEFGHI', $control, 1))
                return true;
        }
    ...
    public function tipoContacto_check($input) {
        ...
        if ($this->Contacto_model->existeTipo($input)) {
            return TRUE;
        } else {
